# Author: Matthias Schinzel (modified by Vincent Palma)
...
class CarGame:
    ...
    def play_game_manually(self):
        ...
        mDQN = torch.load('SavedWeights/dqn.pt',
        ...
        while not self.done:
            ...
            if status[pygame.K_c]:
                self.selfdriving_DQN = False

            if self.selfdriving_DQN is True:
                action = mDQN(torch.FloatTensor([np.append(self.action_space,self.last_actions)])).data.max(1)[1].view(1, 1)
            ...
            print(action,self.current_reward)
